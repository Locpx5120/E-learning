...
public class CourseSectionDAO extends DBContext {
    ...
    public boolean checkExistedComplete(String sectionId, String username) {
        String sql = "select * from course_process where sectionId = ? and username = ?";
        try {
            PreparedStatement pstm = connection.prepareStatement(sql);
    ...
    public void getStatusSection(List<CourseSection> listSection, String username) {
        for (CourseSection courseSection : listSection) {
            setStatus(courseSection, username);
        }
    }

    public void setStatus(CourseSection cs, String username) {
        String sql = "select cs.section_id, cs.description, cs.title, cs.video, cs.course_id, cp.status\n"
                ...
                + "  where cp.sectionid = cs.section_id and cs.section_id = ? and username = ? ";
        try {
            PreparedStatement pstm = connection.prepareStatement(sql);
